...
function EditPost(props) {
  ...
  // Verify valid token, get matching post if ok
  useEffect(() => {
    const checkLoggedIn = async () => {
      ...
      if (tokenRes.data) {
        await axios
          ...
          .then((res) => [setTitle(res.data.title), setContent(res.data.content), setImageName(res.data.imageName), setCloudId(res.data.cloudId)])
          .catch((err) => {
  ...
  const onChangeFile = async (e) => {
    ...
    setImageName(cloudUrlArray);
    setCloudId(cloudIdArray);
  }; //  --- end onChange File

  const onSubmitForm = (e) => {
    e.preventDefault();
    const blogPost = { title, content, imageName, cloudId };
    let token = localStorage.getItem("auth-token");

    axios
      .put(`/posts/update/${props.match.params.id}`, blogPost, { headers: { "x-auth-token": token } })
      .then((res) => [setMessage(res.data.msg), setPostLocation(res.data.id), setMyPosts(true)])
  ...
  const remove = (img, id) => {
    const arr = imageName.filter((item) => item !== img);
    setImageName(arr);
          ...
          <div className=" col col-sm-9 col-lg-6">
            ...
            {title && (
              <>
                ...
                <form action="POST" encType="multipart/form-data" onSubmit={onSubmitForm}>
                  <input value={title} onChange={(e) => setTitle(e.target.value)} className="form-control mb-3" type="text" name="" id="" />
                  <textarea value={content} onChange={(e) => setContent(e.target.value)} className="form-control" type="text" name="" id="" rows="5"></textarea>
